...
import { buildMostLink, parseMostLink } from '~server/src/core/mostLink.js'
...
function getChannelTitle(
  channel?: Pick<
    Channel,
    'remark' | 'channelId' | 'name' | 'directPeerAddress' | 'directPeerName'
  > | null
) {
  return (
    channel?.remark ||
    channel?.directPeerName ||
    (channel?.directPeerAddress
      ? shortAddress(channel.directPeerAddress)
      : getChannelId(channel))
  )
}

function decorateDirectChannel(channel: Channel, contact: DirectContact) {
  return {
    ...channel,
    directPeerAddress: contact.address,
    directPeerName: contact.displayName,
    directStatus: contact.status,
  } satisfies Channel
}

function getDirectAttachmentFromLink(link: string): ChannelAttachment | null {
  const parsed = parseMostLink(link)
  if (parsed.errorCode || !parsed.cid || !parsed.fileName) return null
  return {
    kind: 'file',
    cid: parsed.cid,
    fileName: parsed.fileName,
    link: buildMostLink(parsed.cid, parsed.fileName),
  }
}

function getRequestedChannelNameFromLocation(mode: ChannelMessagingMode) {
  if (typeof window === 'undefined') return ''
  const search = new URLSearchParams(window.location.search)
  if (mode === 'friend') {
    return normalizeDirectAddress(search.get('address'))
  }
  return (search.get('channel') || '').trim()
}
...
export type ChannelMessagingMode = 'public' | 'friend'

interface ChannelMessagingPageProps {
  mode: ChannelMessagingMode
}
...
function ChannelMessagingPage({ mode }: ChannelMessagingPageProps) {
  ...
  const [showAddFriend, addFriendModal] = useDisclosure(false)
  ...
  const [channelMemberProfilesLoadedKey, setChannelMemberProfilesLoadedKey] =
    useState('')
  const isInviteUser = !isFriendMode && userIdentity?.theme === 'sparkbit'
  ...
  const {
    ...
    addContact: addDirectContact,
    removeContact: removeDirectContact,
  } = useDirectContacts({
    isReady: isFriendMode && isBackendReady,
    peerId: myPeerId,
    onError: error => showApiError(error, t('friend.error.sync')),
    onDirectChannelReady: (channel, contact) => {
      const decorated = decorateDirectChannel(channel, contact)
      setChannels(previous => {
        const key = getChannelKey(decorated)
        return previous.some(item => getChannelKey(item) === key)
          ? previous.map(item =>
              getChannelKey(item) === key ? { ...item, ...decorated } : item
            )
          : [...previous, decorated]
      })
    },
  ...
  const directContactsByChannelKey = useMemo(() => {
    const map = new Map<string, DirectContact>()
    if (!userIdentity?.address) return map
    for (const contact of directContacts) {
      map.set(
        buildDirectChannelId(userIdentity.address, contact.address),
        contact
      )
    }
    return map
  }, [directContacts, userIdentity?.address])
  const activeDirectContact = activeChannel
    ? directContactsByChannelKey.get(activeChannelKey) || null
    : null
  const isDirectChat = isFriendMode && activeChannel?.type === 'direct'
  const isDirectChatReady = Boolean(
    isDirectChat && activeDirectContact?.publicKey
  )
  ...
  const activeVoiceRoomInfo = useMemo(() => {
    if (
      !activeChannel ||
      !activeChannelKey ||
      (isDirectChat && !isDirectChatReady)
    ) {
      return null
    }
    return {
      ...
      directPeerAddress: isDirectChat
        ? activeDirectContact?.address
        : undefined,
      directPeerPublicKey: isDirectChat
        ? activeDirectContact?.publicKey
        : undefined,
    }
  }, [
    activeChannel,
  ...
  const displayedChannelMessages = useMemo(() => {
    if (!isDirectChat) return channelMessages
    if (
      !userIdentity ||
      !activeDirectContact?.publicKey ||
      !activeDirectContact.address
    ...
    const selfAddress = normalizeDirectAddress(userIdentity.address)
    return channelMessages.flatMap(message => {
      if (message.type === 'system') return []
      const author = normalizeDirectAddress(message.author)
      if (author !== selfAddress && author !== activeDirectContact.address) {
        return []
      }
      const content = decryptDirectMessage(message.content, {
        identity: userIdentity,
        peerPublicKey: activeDirectContact.publicKey,
        authorAddress: author,
      })
      const attachment = content ? getDirectAttachmentFromLink(content) : null
      return [
        {
          ...message,
          content,
          attachment: attachment || undefined,
          decryptionFailed: !content,
        },
      ]
    })
  }, [activeDirectContact, channelMessages, isDirectChat, userIdentity])
  ...
  useEffect(() => {
    const updateRequestedChannelName = () => {
      setRequestedChannelName(getRequestedChannelNameFromLocation(mode))
    }
  ...
  useEffect(() => {
    ...
    void refreshChannels()
  }, [directContactsByChannelKey, isFriendMode])
  ...
  useEffect(() => {
    ...
    const found = isFriendMode
      ? channels.find(
          channel =>
            normalizeDirectAddress(channel.directPeerAddress) ===
    ...
    if (isDirectSystemChannel(requestedChannelName, DIRECT_CHANNEL_TYPE)) {
      return
    }
  ...
  }, [clearChannelMessages, mode, userIdentity?.address])
  ...
  function getFriendAddressValidationError(address: string) {
    const normalized = normalizeDirectAddress(address)
    if (!normalized) return t('friend.validation.address')
    if (normalized === normalizeDirectAddress(userIdentity?.address)) {
      return t('friend.validation.self')
    }
    return ''
  }
  ...
  async function refreshChannels() {
    ...
    try {
      const result = isFriendMode
        ? (await channelApi.getChannels({ type: 'direct' })).flatMap(
            channel => {
              const contact = directContactsByChannelKey.get(
                getChannelKey(channel)
              )
              return contact ? [decorateDirectChannel(channel, contact)] : []
            }
          )
  ...
  async function handleOpenChannel(channel: Channel) {
    ...
    const requestedName = isFriendMode
      ? normalizeDirectAddress(channel.directPeerAddress)
      : channelKey
    setRequestedChannelName(requestedName)
    window.history.pushState(
      {},
      '',
      isFriendMode
        ? `?address=${encodeURIComponent(requestedName)}`
  ...
  async function handleLeaveChannel(
    ...
    const leavingChannel =
      channels.find(channel => getChannelKey(channel) === channelKey) ||
      (getChannelKey(activeChannel) === channelKey ? activeChannel : null)
    setIsLeavingChannel(true)
    try {
      await channelApi.leaveChannel(channelKey)
      if (
        leavingChannel?.type === 'direct' &&
        leavingChannel.directPeerAddress
      ) {
        removeDirectContact(leavingChannel.directPeerAddress)
      }
      if (getChannelKey(activeChannel) === channelKey) {
        ...
        url.searchParams.delete(isFriendMode ? 'address' : 'channel')
  ...
  async function handleAddFriend(address: string) {
    const normalized = normalizeDirectAddress(address)
    if (!normalized || isAddingFriend) return
    const validationError = getFriendAddressValidationError(normalized)
    if (validationError) {
      addToast(validationError, 'error')
      return
    }
    ...
    try {
      const { channel, contact } = await addDirectContact(normalized)
      const decorated = decorateDirectChannel(channel, contact)
      const channelKey = getChannelKey(decorated)
      setChannels(previous =>
        previous.some(item => getChannelKey(item) === channelKey)
          ? previous.map(item =>
      ...
      await handleOpenChannel(decorated)
      addToast(
        contact.status === 'ready' ? t('friend.added') : t('friend.pending'),
        'success'
      )
    } catch (error) {
      await showApiError(error, t('friend.error.add'))
    } finally {
  ...
  async function sendChannelMessage(
    ...
    const activeChannelKey = getChannelKey(activeChannel)
    let transmittedContent = trimmedContent
    let transmittedAttachment = attachment
    let transmittedMentions = mentions

    if (activeChannel.type === 'direct') {
      if (!activeDirectContact?.publicKey) {
        addToast(t('friend.pending'), 'error')
        return false
      }
      try {
        transmittedContent = encryptDirectMessage(
          trimmedContent,
          userIdentity,
          activeDirectContact.publicKey
        )
        ...
        if (error instanceof RangeError) {
          addToast(
            t('friend.validation.messageMax', {
              count: DIRECT_MESSAGE_MAX_LENGTH,
            }),
            'error'
          )
          return false
        }
        await showApiError(error, t('friend.error.encrypt'))
        return false
      }
      transmittedAttachment = undefined
      transmittedMentions = []
    }
    ...
    try {
      const sentMessage = await sendSharedChannelMessage({
        ...
        attachment: transmittedAttachment,
        mentions:
          !transmittedAttachment && transmittedMentions.length > 0
            ? transmittedMentions
            : undefined,
      })
      setChannels(prev =>
  ...
  function renderMessageBubble(msg: ChannelMessage) {
    if (msg.decryptionFailed) {
      return (
        <ChatTextBubble>
          <span className="chat-direct-decryption-error">
            {t('friend.decryptFailed')}
          </span>
        </ChatTextBubble>
      )
    }
  ...
  const chatHeaderTitle = activeChannel ? (
    <div className="chat-header-title-row">
      ...
      {isDirectChat && (
        <LockKeyhole
          className={isDirectChatReady ? 'is-ready' : 'is-pending'}
          size={15}
          aria-label={
            isDirectChatReady ? t('friend.encrypted') : t('friend.pending')
          }
        />
      )}
    </div>
    ...
    <h2 className="header-title">
      {t(isFriendMode ? 'friend.title' : 'chat.title')}
    </h2>
  )
  ...
  return (
            ...
            {channels.length === 0 ? (
              <div className="sidebar-empty-state">
                <p>
                  {t(
                    isFriendMode
                      ? 'friend.empty.noFriends'
                      : 'chat.empty.noChannels'
                  )}
                </p>
              </div>
              ...
              <div className="sidebar-empty-state">
                <p>
                  {t(
                    isFriendMode
                      ? 'friend.empty.noMatches'
                      : 'chat.empty.noMatches'
                  )}
                </p>
              </div>
            ) : (
    ...
    >
      {shouldShowChatRestoring ? (
        ...
        <>
          ...
          <div className="chat-messages">
            {displayedChannelMessages.length === 0 ? (
              <div className="ui-empty-state chat-messages-empty">
                ...
                <p>
                  {isDirectChat && !isDirectChatReady
                    ...
                    : t(
                        isFriendMode
                          ? 'friend.empty.noMessages'
                          : 'chat.empty.noMessages'
                      )}
                </p>
              </div>
            ) : (
              displayedChannelMessages.map(msg => {
          ...
          <ChatComposer
            ...
            placeholder={
              userIdentity
                ? isDirectChat && !isDirectChatReady
                  ? t('friend.composer.pending')
                  : t('chat.composer.placeholder')
                : t('chat.composer.signInPlaceholder')
            }
            disabled={!userIdentity || (isDirectChat && !isDirectChatReady)}
            ...
            mentionMenu={isDirectChat ? undefined : mentionMenu}
        ...
        <>
          <div className="ui-empty-state chat-welcome">
            ...
            <h2 className="ui-empty-title">
              {t(isFriendMode ? 'friend.select.title' : 'chat.select.title')}
            </h2>
            <p className="ui-empty-desc">
              {t(isFriendMode ? 'friend.select.desc' : 'chat.select.desc')}
            </p>
            <OpenSidebarButton
              label={t(
                isFriendMode ? 'friend.openList' : 'chat.openChannelList'
              )}
            />
          </div>
        </>
      ...
      {isFriendMode && showAddFriend && (
        <InputModal
          title={t('friend.addFriend')}
          placeholder={t('friend.addressPlaceholder')}
          confirmText={t('friend.add')}
          validate={getFriendAddressValidationError}
          onConfirm={handleAddFriend}
          onClose={() => addFriendModal.close()}
          isLoading={isAddingFriend}
          loadingText={t('friend.adding')}
        />
      )}
      ...
      {showLeaveChannelConfirm && channelToLeave && (
        <ConfirmModal
          title={isFriendMode ? t('friend.remove') : t('chat.leaveChannel')}
          message={
            isFriendMode
              ? t('friend.removeConfirm', {
                  friend: getChannelTitle(channelToLeave),
                })
              : t('chat.leaveConfirm', {
          ...
          >
              ...
              {!isInviteUser && (
                <div className="channel-detail-section">
                  ...
                  <div
                    ...
                    translate="no"
                  >
                    {isDirectChat && activeChannel.directPeerAddress
                      ? activeChannel.directPeerAddress
                      : getChannelId(activeChannel)}
                  </div>
                </div>
              )}
            ...
            {!isInviteUser && (
              <div className="channel-detail-footer">
                ...
                >
                  {isDirectChat ? t('friend.remove') : t('chat.leaveChannel')}
                </button>
              </div>
            )}
          </div>
