export const nodeMessages = {
  'node.status.idle': {
    'zh-CN': '未启动',
    'zh-TW': '未啟動',
    en: 'Not started',
  },
  'node.status.starting': {
    'zh-CN': '启动中',
    'zh-TW': '啟動中',
    en: 'Starting',
  },
  'node.status.ready': {
    'zh-CN': '在线',
    'zh-TW': '在線',
    en: 'Online',
  },
  'node.status.stopping': {
    'zh-CN': '停止中',
    'zh-TW': '停止中',
    en: 'Stopping',
  },
  'node.status.error': {
    'zh-CN': '异常',
    'zh-TW': '異常',
    en: 'Error',
  },
  'node.seed.queued': {
    'zh-CN': '队列中',
    'zh-TW': '佇列中',
    en: 'Queued',
  },
  'node.seed.joining': {
    'zh-CN': '加入中',
    'zh-TW': '加入中',
    en: 'Joining',
  },
  'node.seed.active': {
    'zh-CN': '做种中',
    'zh-TW': '做種中',
    en: 'Seeding',
  },
  'node.seed.paused': {
    'zh-CN': '已暂停',
    'zh-TW': '已暫停',
    en: 'Paused',
  },
  'node.seed.error': {
    'zh-CN': '错误',
    'zh-TW': '錯誤',
    en: 'Error',
  },
  'node.transfer.queued': {
    'zh-CN': '排队中',
    'zh-TW': '排隊中',
    en: 'Queued',
  },
  'node.transfer.running': {
    'zh-CN': '传输中',
    'zh-TW': '傳輸中',
    en: 'Transferring',
  },
  'node.transfer.completed': {
    'zh-CN': '已完成',
    'zh-TW': '已完成',
    en: 'Completed',
  },
  'node.transfer.failed': {
    'zh-CN': '失败',
    'zh-TW': '失敗',
    en: 'Failed',
  },
  'node.transfer.waitingCore': {
    'zh-CN': '等待核心',
    'zh-TW': '等待核心',
    en: 'Waiting for core',
  },
  'node.transfer.publish': {
    'zh-CN': '发布',
    'zh-TW': '發佈',
    en: 'Publish',
  },
  'node.transfer.download': {
    'zh-CN': '下载',
    'zh-TW': '下載',
    en: 'Download',
  },
  'node.log.info': {
    'zh-CN': '信息',
    'zh-TW': '資訊',
    en: 'Info',
  },
  'node.log.warn': {
    'zh-CN': '提醒',
    'zh-TW': '提醒',
    en: 'Warning',
  },
  'node.log.error': {
    'zh-CN': '错误',
    'zh-TW': '錯誤',
    en: 'Error',
  },
  'node.transfer.errorDetails': {
    'zh-CN': '错误详情',
    'zh-TW': '錯誤詳情',
    en: 'Error details',
  },
  'node.transfer.retrying': {
    'zh-CN': '重试中',
    'zh-TW': '重試中',
    en: 'Retrying',
  },
  'node.transfer.redownload': {
    'zh-CN': '重新下载',
    'zh-TW': '重新下載',
    en: 'Download again',
  },
  'node.transfer.reselect': {
    'zh-CN': '重新选择文件',
    'zh-TW': '重新選擇檔案',
    en: 'Choose file again',
  },
  'node.action.publish': {
    'zh-CN': '发布文件',
    'zh-TW': '發佈檔案',
    en: 'Publish file',
  },
  'node.action.receive': {
    'zh-CN': '接收文件',
    'zh-TW': '接收檔案',
    en: 'Receive file',
  },
  'node.section.status': {
    'zh-CN': '节点状态',
    'zh-TW': '節點狀態',
    en: 'Node status',
  },
  'node.metric.onlinePeers': {
    'zh-CN': '在线 Peer',
    'zh-TW': '在線 Peer',
    en: 'Online peers',
  },
  'node.metric.localSeeds': {
    'zh-CN': '本机做种',
    'zh-TW': '本機做種',
    en: 'Local seeds',
  },
  'node.metric.remoteFiles': {
    'zh-CN': '远程文件',
    'zh-TW': '遠端檔案',
    en: 'Remote files',
  },
  'node.metric.attachmentVerification': {
    'zh-CN': '附件校验',
    'zh-TW': '附件驗證',
    en: 'Attachment checks',
  },
  'node.metric.enabled': {
    'zh-CN': '开启',
    'zh-TW': '開啟',
    en: 'On',
  },
  'node.section.knowledge': {
    'zh-CN': '知识库',
    'zh-TW': '知識庫',
    en: 'Knowledge base',
  },
  'node.knowledge.backup': {
    'zh-CN': '备份',
    'zh-TW': '備份',
    en: 'Back up',
  },
  'node.knowledge.backupA11y': {
    'zh-CN': '备份知识库',
    'zh-TW': '備份知識庫',
    en: 'Back up knowledge base',
  },
  'node.knowledge.restore': {
    'zh-CN': '还原',
    'zh-TW': '還原',
    en: 'Restore',
  },
  'node.knowledge.restoreA11y': {
    'zh-CN': '还原知识库',
    'zh-TW': '還原知識庫',
    en: 'Restore knowledge base',
  },
  'node.section.seeding': {
    'zh-CN': '正在做种',
    'zh-TW': '正在做種',
    en: 'Seeding',
  },
  'node.fileCount': {
    'zh-CN': '{count} 个文件',
    'zh-TW': '{count} 個檔案',
    en: '{count} files',
  },
  'node.fileCount.one': {
    'zh-CN': '{count} 个文件',
    'zh-TW': '{count} 個檔案',
    en: '{count} file',
  },
  'node.holding.topicJoined': {
    'zh-CN': 'Topic 已加入',
    'zh-TW': 'Topic 已加入',
    en: 'Topic joined',
  },
  'node.holding.topicWaiting': {
    'zh-CN': '等待加入 topic',
    'zh-TW': '等待加入 topic',
    en: 'Waiting to join topic',
  },
  'node.action.copied': {
    'zh-CN': '已复制',
    'zh-TW': '已複製',
    en: 'Copied',
  },
  'node.action.copyLink': {
    'zh-CN': '复制链接',
    'zh-TW': '複製連結',
    en: 'Copy link',
  },
  'node.action.processing': {
    'zh-CN': '处理中',
    'zh-TW': '處理中',
    en: 'Processing',
  },
  'node.action.saveToFiles': {
    'zh-CN': '存到文件',
    'zh-TW': '儲存到檔案',
    en: 'Save to Files',
  },
  'node.action.deleting': {
    'zh-CN': '删除中',
    'zh-TW': '刪除中',
    en: 'Deleting',
  },
  'node.empty.filesTitle': {
    'zh-CN': '还没有本机文件',
    'zh-TW': '尚無本機檔案',
    en: 'No local files yet',
  },
  'node.empty.filesBody': {
    'zh-CN': '发布或下载完成后，文件会自动加入做种列表。',
    'zh-TW': '發佈或下載完成後，檔案會自動加入做種清單。',
    en: 'Published and downloaded files are automatically added to the seeding list.',
  },
  'node.section.activeTransfers': {
    'zh-CN': '进行中',
    'zh-TW': '進行中',
    en: 'In progress',
  },
  'node.taskCount': {
    'zh-CN': '{count} 个任务',
    'zh-TW': '{count} 個任務',
    en: '{count} tasks',
  },
  'node.taskCount.one': {
    'zh-CN': '{count} 个任务',
    'zh-TW': '{count} 個任務',
    en: '{count} task',
  },
  'node.empty.transfersTitle': {
    'zh-CN': '当前没有传输',
    'zh-TW': '目前沒有傳輸',
    en: 'No current transfers',
  },
  'node.empty.transfersBody': {
    'zh-CN': '新的发布和下载任务会显示在这里。',
    'zh-TW': '新的發佈和下載任務會顯示在這裡。',
    en: 'New publish and download tasks will appear here.',
  },
  'node.section.failedTransfers': {
    'zh-CN': '需要处理',
    'zh-TW': '需要處理',
    en: 'Needs attention',
  },
  'node.failedTaskCount': {
    'zh-CN': '{count} 个失败任务',
    'zh-TW': '{count} 個失敗任務',
    en: '{count} failed tasks',
  },
  'node.failedTaskCount.one': {
    'zh-CN': '{count} 个失败任务',
    'zh-TW': '{count} 個失敗任務',
    en: '{count} failed task',
  },
  'node.section.completedTransfers': {
    'zh-CN': '最近完成',
    'zh-TW': '最近完成',
    en: 'Recently completed',
  },
  'node.recordCount': {
    'zh-CN': '{count} 条记录',
    'zh-TW': '{count} 筆記錄',
    en: '{count} records',
  },
  'node.recordCount.one': {
    'zh-CN': '{count} 条记录',
    'zh-TW': '{count} 筆記錄',
    en: '{count} record',
  },
  'node.diagnostics.title': {
    'zh-CN': '诊断详情',
    'zh-TW': '診斷詳情',
    en: 'Diagnostics',
  },
  'node.diagnostics.recentLogs': {
    'zh-CN': '最近 {count} 条日志',
    'zh-TW': '最近 {count} 筆日誌',
    en: '{count} recent logs',
  },
  'node.diagnostics.noLogs': {
    'zh-CN': '暂无日志',
    'zh-TW': '暫無日誌',
    en: 'No logs',
  },
  'node.diagnostics.emptyTitle': {
    'zh-CN': '日志为空',
    'zh-TW': '日誌為空',
    en: 'No log entries',
  },
  'node.diagnostics.emptyBody': {
    'zh-CN': '核心状态变化和传输事件会记录在这里。',
    'zh-TW': '核心狀態變化和傳輸事件會記錄在這裡。',
    en: 'Core status changes and transfer events are recorded here.',
  },
  'node.about.title': {
    'zh-CN': '关于与政策',
    'zh-TW': '關於與政策',
    en: 'About and policies',
  },
  'node.about.privacy': {
    'zh-CN': '隐私政策',
    'zh-TW': '隱私權政策',
    en: 'Privacy Policy',
  },
  'node.about.terms': {
    'zh-CN': '使用条款',
    'zh-TW': '使用條款',
    en: 'Terms of Use',
  },
  'node.about.support': {
    'zh-CN': '问题反馈',
    'zh-TW': '問題回報',
    en: 'Support',
  },
  'node.connection.title': {
    'zh-CN': '节点连接',
    'zh-TW': '節點連線',
    en: 'Node connection',
  },
  'node.connection.local': {
    'zh-CN': '本机节点',
    'zh-TW': '本機節點',
    en: 'Local node',
  },
  'node.connection.remote': {
    'zh-CN': '远程节点',
    'zh-TW': '遠端節點',
    en: 'Remote node',
  },
  'node.connection.manage': {
    'zh-CN': '管理连接',
    'zh-TW': '管理連線',
    en: 'Manage connection',
  },
  'node.connection.modalTitle': {
    'zh-CN': '连接节点',
    'zh-TW': '連線節點',
    en: 'Connect node',
  },
  'node.connection.url': {
    'zh-CN': '节点地址',
    'zh-TW': '節點地址',
    en: 'Node URL',
  },
  'node.connection.invite': {
    'zh-CN': '邀请码',
    'zh-TW': '邀請碼',
    en: 'Invite code',
  },
  'node.connection.connect': {
    'zh-CN': '连接远程节点',
    'zh-TW': '連線遠端節點',
    en: 'Connect remote node',
  },
  'node.connection.connecting': {
    'zh-CN': '正在连接',
    'zh-TW': '正在連線',
    en: 'Connecting',
  },
  'node.connection.switchLocal': {
    'zh-CN': '切回本机节点',
    'zh-TW': '切回本機節點',
    en: 'Switch to local node',
  },
  'node.connection.history': {
    'zh-CN': '历史节点',
    'zh-TW': '歷史節點',
    en: 'Node history',
  },
  'node.connection.current': {
    'zh-CN': '当前',
    'zh-TW': '目前',
    en: 'Current',
  },
  'node.connection.fallback': {
    'zh-CN': '远程节点不可达，当前已回退到本机',
    'zh-TW': '遠端節點無法連線，目前已回退到本機',
    en: 'Remote node unavailable; using the local node',
  },
  'node.connection.error.invalidUrl': {
    'zh-CN': '请输入有效的 HTTP 或 HTTPS 节点地址',
    'zh-TW': '請輸入有效的 HTTP 或 HTTPS 節點地址',
    en: 'Enter a valid HTTP or HTTPS node URL',
  },
  'node.connection.error.required': {
    'zh-CN': '请先连接一个远程 MostBox 节点',
    'zh-TW': '請先連線一個遠端 MostBox 節點',
    en: 'Connect to a remote MostBox node first',
  },
  'node.connection.error.invalidInvite': {
    'zh-CN': '邀请码无效或远程访问未启用',
    'zh-TW': '邀請碼無效或遠端存取未啟用',
    en: 'The invite code is invalid or remote access is not enabled',
  },
  'node.connection.error.identity': {
    'zh-CN': '远程节点未接受当前登录身份',
    'zh-TW': '遠端節點未接受目前登入身分',
    en: 'The remote node did not accept the signed-in identity',
  },
  'node.connection.error.switchBlocked': {
    'zh-CN': '请先完成或取消当前发布和下载',
    'zh-TW': '請先完成或取消目前發佈和下載',
    en: 'Finish or cancel active transfers before switching nodes',
  },
  'node.connection.error.unreachable': {
    'zh-CN': '无法连接远程节点，请检查地址、邀请码和网络',
    'zh-TW': '無法連線遠端節點，請檢查地址、邀請碼和網路',
    en: 'Could not connect to the remote node. Check the URL, invite, and network',
  },
  'node.connection.error.generic': {
    'zh-CN': '节点操作未完成，请稍后重试',
    'zh-TW': '節點操作未完成，請稍後重試',
    en: 'The node operation did not complete. Try again later',
  },
  'node.account.title': {
    'zh-CN': '远程账号',
    'zh-TW': '遠端帳號',
    en: 'Remote account',
  },
  'node.account.signedOut': {
    'zh-CN': '未登录',
    'zh-TW': '未登入',
    en: 'Signed out',
  },
  'node.account.username': {
    'zh-CN': '用户名',
    'zh-TW': '使用者名稱',
    en: 'Username',
  },
  'node.account.password': {
    'zh-CN': '密码',
    'zh-TW': '密碼',
    en: 'Password',
  },
  'node.account.signIn': {
    'zh-CN': '登录',
    'zh-TW': '登入',
    en: 'Sign in',
  },
  'node.account.signingIn': {
    'zh-CN': '正在登录',
    'zh-TW': '正在登入',
    en: 'Signing in',
  },
  'node.account.signOut': {
    'zh-CN': '退出登录',
    'zh-TW': '登出',
    en: 'Sign out',
  },
  'node.account.signOutConfirmTitle': {
    'zh-CN': '退出远程节点账号？',
    'zh-TW': '登出遠端節點帳號？',
    en: 'Sign out of the remote node?',
  },
  'node.account.signOutConfirmBody': {
    'zh-CN': '退出后需要重新登录才能继续使用受保护的节点操作。',
    'zh-TW': '登出後需要重新登入才能繼續使用受保護的節點操作。',
    en: 'You will need to sign in again to use protected node operations.',
  },
  'node.account.required': {
    'zh-CN': '登录后可使用远程文件',
    'zh-TW': '登入後可使用遠端檔案',
    en: 'Sign in to use remote files',
  },
  'files.search.a11y': {
    'zh-CN': '搜索文件',
    'zh-TW': '搜尋檔案',
    en: 'Search files',
  },
  'files.search.placeholder': {
    'zh-CN': '搜索名称或 CID',
    'zh-TW': '搜尋名稱或 CID',
    en: 'Search name or CID',
  },
  'files.search.clear': {
    'zh-CN': '清除文件搜索',
    'zh-TW': '清除檔案搜尋',
    en: 'Clear file search',
  },
  'files.section.library': {
    'zh-CN': '文件库',
    'zh-TW': '檔案庫',
    en: 'File library',
  },
  'files.entryCount': {
    'zh-CN': '{count} 项',
    'zh-TW': '{count} 項',
    en: '{count} items',
  },
  'files.entryCount.one': {
    'zh-CN': '{count} 项',
    'zh-TW': '{count} 項',
    en: '{count} item',
  },
  'files.folder.type': {
    'zh-CN': '文件夹',
    'zh-TW': '資料夾',
    en: 'Folder',
  },
  'files.folder.openHint': {
    'zh-CN': '进入文件夹',
    'zh-TW': '進入資料夾',
    en: 'Open folder',
  },
  'files.folder.shareLabel': {
    'zh-CN': '分享文件夹 {name}',
    'zh-TW': '分享資料夾 {name}',
    en: 'Share folder {name}',
  },
  'files.folder.shareDetailsLabel': {
    'zh-CN': '管理文件夹 {name} 的分享',
    'zh-TW': '管理資料夾 {name} 的分享',
    en: 'Manage the share for folder {name}',
  },
  'files.folder.shared': {
    'zh-CN': '文件夹分享链接已复制',
    'zh-TW': '資料夾分享連結已複製',
    en: 'Folder share link copied',
  },
  'files.folder.shareFailedTitle': {
    'zh-CN': '无法分享文件夹',
    'zh-TW': '無法分享資料夾',
    en: 'Could not share folder',
  },
  'files.folder.shareRequiresLocalFiles': {
    'zh-CN': '文件夹中有尚未下载到远程节点本机的文件，请先下载后再分享。',
    'zh-TW': '資料夾中有尚未下載到遠端節點本機的檔案，請先下載後再分享。',
    en: 'Download every file in this folder to the remote node before sharing it.',
  },
  'files.folder.shareEmpty': {
    'zh-CN': '这个文件夹里没有可分享的本地文件。',
    'zh-TW': '這個資料夾裡沒有可分享的本機檔案。',
    en: 'This folder has no local files to share.',
  },
  'files.collection.meta': {
    'zh-CN': '{count} 个文件 · {size}',
    'zh-TW': '{count} 個檔案 · {size}',
    en: '{count} files · {size}',
  },
  'files.filter.all': {
    'zh-CN': '全部',
    'zh-TW': '全部',
    en: 'All',
  },
  'files.filter.active': {
    'zh-CN': '做种中',
    'zh-TW': '做種中',
    en: 'Seeding',
  },
  'files.filter.attention': {
    'zh-CN': '需处理',
    'zh-TW': '需處理',
    en: 'Attention',
  },
  'files.details.hint': {
    'zh-CN': '打开文件详情和操作',
    'zh-TW': '開啟檔案詳情和操作',
    en: 'Open file details and actions',
  },
  'files.details.peers': {
    'zh-CN': '{count} 个 Peer',
    'zh-TW': '{count} 個 Peer',
    en: '{count} peers',
  },
  'files.details.path': {
    'zh-CN': '路径',
    'zh-TW': '路徑',
    en: 'Path',
  },
  'files.action.open': {
    'zh-CN': '打开',
    'zh-TW': '開啟',
    en: 'Open',
  },
  'files.action.downloadToLibrary': {
    'zh-CN': '下载到文件库',
    'zh-TW': '下載到檔案庫',
    en: 'Download to file library',
  },
  'files.empty.searchTitle': {
    'zh-CN': '没有匹配的文件',
    'zh-TW': '沒有符合的檔案',
    en: 'No matching files',
  },
  'files.empty.searchBody': {
    'zh-CN': '调整搜索内容或筛选条件。',
    'zh-TW': '調整搜尋內容或篩選條件。',
    en: 'Change the search or filter.',
  },
  'files.empty.libraryTitle': {
    'zh-CN': '文件库暂无文件',
    'zh-TW': '檔案庫暫無檔案',
    en: 'No files in the library',
  },
  'files.empty.libraryBody': {
    'zh-CN': '发布或下载文件后会显示在这里。',
    'zh-TW': '發佈或下載檔案後會顯示在這裡。',
    en: 'Published and downloaded files appear here.',
  },
  'transfers.view.active': {
    'zh-CN': '进行中',
    'zh-TW': '進行中',
    en: 'Active',
  },
  'transfers.view.completed': {
    'zh-CN': '已完成',
    'zh-TW': '已完成',
    en: 'Completed',
  },
  'transfers.view.failed': {
    'zh-CN': '失败',
    'zh-TW': '失敗',
    en: 'Failed',
  },
  'transfers.cancel': {
    'zh-CN': '取消下载',
    'zh-TW': '取消下載',
    en: 'Cancel download',
  },
  'transfers.cancelling': {
    'zh-CN': '正在取消',
    'zh-TW': '正在取消',
    en: 'Cancelling',
  },
  'transfers.openFile': {
    'zh-CN': '点击查看文件详情',
    'zh-TW': '點擊查看檔案詳情',
    en: 'Tap to view file details',
  },
  'transfers.empty.activeTitle': {
    'zh-CN': '当前没有传输',
    'zh-TW': '目前沒有傳輸',
    en: 'No active transfers',
  },
  'transfers.empty.activeBody': {
    'zh-CN': '新的发布和下载任务会显示在这里。',
    'zh-TW': '新的發佈和下載任務會顯示在這裡。',
    en: 'New publishes and downloads will appear here.',
  },
  'transfers.empty.completedTitle': {
    'zh-CN': '还没有完成记录',
    'zh-TW': '尚無完成記錄',
    en: 'No completed transfers',
  },
  'transfers.empty.completedBody': {
    'zh-CN': '校验完成的发布和下载会保留在这里。',
    'zh-TW': '驗證完成的發佈和下載會保留在這裡。',
    en: 'Verified publishes and downloads will remain here.',
  },
  'transfers.empty.failedTitle': {
    'zh-CN': '没有失败任务',
    'zh-TW': '沒有失敗任務',
    en: 'No failed transfers',
  },
  'transfers.empty.failedBody': {
    'zh-CN': '需要重试的任务会显示在这里。',
    'zh-TW': '需要重試的任務會顯示在這裡。',
    en: 'Transfers that need a retry will appear here.',
  },
} as const
