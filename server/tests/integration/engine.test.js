...
describe('MostBoxEngine (integration)', { timeout: 420000 }, () => {
  ...
  describe('node holdings and CID topic pull', () => {
    ...
    it('downloads CID content to an internal path without publishing metadata', async () => {
      const updateTmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), 'most-update-cid-')
      )
      let publisher
      let downloader
      let replication
      ...
      try {
        publisher = new MostBoxEngine({
          dataPath: path.join(updateTmpDir, 'publisher'),
          downloadTimeout: 5000,
        })
        downloader = new MostBoxEngine({
          dataPath: path.join(updateTmpDir, 'downloader'),
          downloadTimeout: 5000,
        })
        await publisher.start()
        await downloader.start()

        const content = Buffer.from('desktop update payload')
        const published = await publisher.publishFile(content, 'MostBox-test.exe')
        ...
        const download = downloader.downloadCidToPath({
          cid: published.cid,
        ...
        assert.strictEqual(result.cid, published.cid)
        assert.strictEqual(result.savedPath, targetPath)
        assert.deepStrictEqual(fs.readFileSync(targetPath), content)
        assert.deepStrictEqual(downloader.listPublishedFiles(), [])
        assert.ok(
          downloader
            .listHoldings()
            .some(
              holding =>
                holding.cid === published.cid && holding.source === 'update'
            )
        )
      } finally {
        replication?.close()
    ...
    it('rejects internal seed files whose content does not match the CID', async () => {
      const seedTmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), 'most-update-seed-')
      )
      const expected = await calculateCid(Buffer.from('expected update'))
      const filePath = path.join(seedTmpDir, 'wrong-update.bin')

      try {
        fs.writeFileSync(filePath, 'wrong update')

        await assert.rejects(
          engine.seedCidFileFromPath(expected.cid.toString(), filePath, {
            fileName: 'wrong-update.bin',
            source: 'update',
          }),
          /CID mismatch/
        )
        assert.ok(
          !engine
            .listHoldings()
            .some(holding => holding.cid === expected.cid.toString())
        )
      } finally {
