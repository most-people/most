...
export class MostBoxEngine extends EventEmitter {
  ...
  /**
   * 按 CID 将内容下载到指定路径，不写入用户文件列表。
   * 用于桌面客户端更新等内部缓存场景。
   * @param {object} input - 下载参数
   * @param {string} input.cid - 文件 CID
   * @param {string} input.targetPath - 目标文件路径
   * @param {string} [input.fileName] - 持有记录展示名
   * @param {string} [input.taskId] - 下载任务 ID
   * @param {number} [input.timeout] - 等待 P2P 内容的超时时间
   * @param {number} [input.streamReadTimeout] - 下载流无进度超时时间
   * @param {boolean} [input.overwrite=true] - 是否覆盖已存在目标文件
   * @param {string} [input.source='update'] - holding 来源
   */
  async downloadCidToPath(input = {}) {
    this.#ensureInitialized()
    ...
    this.#getCidInfo(cid)
    const taskId =
      input.taskId || `cid_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
    const taskState = { aborted: false, readStream: null, writeStream: null }
    const timeout = input.timeout || this.#options.downloadTimeout
    const streamReadTimeout =
      input.streamReadTimeout ?? STREAM_READ_TIMEOUT
    const fileName = sanitizeFilename(input.fileName || path.basename(targetPath))
    const overwrite = input.overwrite !== false
    const resolvedTargetPath = path.resolve(targetPath)
    const targetDir = path.dirname(resolvedTargetPath)
    const tempPath = `${resolvedTargetPath}.part`
    ...
    try {
      ...
      if (!overwrite && fs.existsSync(resolvedTargetPath)) {
        throw new ConflictError(`File already exists: ${resolvedTargetPath}`)
      }
      fs.rmSync(tempPath, { force: true })

      const { driveName } = this.#getCidInfo(cid)
      let drive = this.#drives.get(driveName)
      if (!drive) {
      ...
      await this.#joinCidTopicInternal(cid, {
        server: false,
        client: true,
      })
      ...
      try {
        const stat = await drive.entry(entry.key)
        totalBytes = Number(stat?.value?.blob?.byteLength) || 0
      } catch {}
      ...
      await new Promise((resolve, reject) => {
        ...
        rs.on('error', fail)
        rs.on('close', () => {
          if (taskState.aborted) fail(new Error('Download cancelled'))
        })
        ws.on('close', () => {
          if (taskState.aborted) fail(new Error('Download cancelled'))
        })
      ...
      this.emit('download:status', { taskId, status: 'verifying' })
      const { cid: downloadedCid, size } = await calculateCid(tempPath)
      if (downloadedCid.toString() !== cid) {
        fs.unlinkSync(tempPath)
        throw new IntegrityError(
          `File content CID mismatch. Expected ${cid}, got ${downloadedCid.toString()}.`
        )
      }
      ...
      await this.seedCidFileFromPath(cid, resolvedTargetPath, {
        fileName,
  ...
  /**
   * 将本地文件按既有 CID 写入 Hyperdrive 并记录 holding，不写入用户文件列表。
   * @param {string} cid - 文件 CID
   * @param {string} filePath - 本地文件路径
   * @param {object} [options] - 选项
   * @param {string} [options.fileName] - 持有记录展示名
   * @param {string} [options.source='update'] - holding 来源
   */
  async seedCidFileFromPath(cid, filePath, options = {}) {
    this.#ensureInitialized()
    if (!cid) throw new ValidationError('cid is required')
    ...
    const { cid: actualCid } = await calculateCid(filePath)
    const actualCidString = actualCid.toString()
    if (actualCidString !== cid) {
      throw new IntegrityError(
        `File content CID mismatch. Expected ${cid}, got ${actualCidString}.`
      )
    }

    const existingHolding = this.#holdings.find(holding => holding.cid === cid)
    if (!existingHolding) {
      this.#checkCapacity(sizeValidation.size || 0)
    }
    ...
    return this.#upsertHolding({
      cid,
      fileName: sanitizeFilename(options.fileName || path.basename(filePath)),
      size: sizeValidation.size || 0,
      driveName,
