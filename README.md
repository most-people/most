...
5. **P2P 频道聊天**
   - 默认创建 128 位随机频道 ID，也可使用 3-30 位自定义 ID
...
- 可以生成 16 字节、22 位 base64url 频道 ID
- 也可以输入符合 `[A-Za-z0-9_-]{3,30}` 的自定义 ID
...
1. 打开 `/chat/`，点击「打开频道」
2. 输入已有频道 ID/分享链接，或点击随机按钮生成一个新 ID
3. 打开频道；已有 ID 会加入对应频道，新随机 ID 会创建频道
