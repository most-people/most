...
运行方式：`守护进程 (Core) + 浏览器客户端`。启动后自动打开浏览器访问 `http://127.0.0.1:1976`。
...
2. 运行 `MostBox-xxx-win.exe` 安装
3. 安装完成后自动启动，浏览器打开 `http://127.0.0.1:1976`

**Windows 用户**：首次运行可能会被 SmartScreen 拦截，点击"仍要运行"即可。

**macOS 用户**：首次运行需要在终端执行：
```bash
xattr -d com.apple.quarantine /Applications/MostBox.app
```
