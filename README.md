...
services:
  mostbox:
    image: ghcr.io/most-people/most-box:0.4.6
...
登录创建旧笔记时使用的原账号，从知识库进入 `/note/decrypt/` 临时迁移工具。覆盖保存前，浏览器知识库应先在个人资料中下载账号备份；桌面 Markdown 笔记库还应手动复制整个目录。工具只把能够确认解密成功的 `mp://1` 旧文章保存为普通 Markdown，密钥不匹配、内容损坏或扫描后被外部修改的文章保持原样。账号备份文件本身仍整体加密。
