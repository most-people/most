...
Most.Box 不彻底改成单一聊天产品，而是升级为 **聊天优先的 P2P 工具箱**：用户之间直接连接，从聊天开始，在聊天里完成通信、传文件、做记录和开游戏房间。

对普通用户的表达：

```text
Most.Box：用户之间直接连接的 P2P 工具箱。
从聊天开始，直接传文件、做记录、开游戏房间。
```
...
```text
Chat first.
Channel 是消息、文件附件、记录入口和游戏房间的共同承载层。
文件分享、知识库和游戏保留独立能力，但产品入口围绕聊天组织。
```
...
| 功能组织 | 文件、记录、游戏都从聊天自然进入 | 一堆互不相关的独立工具 |
...
| 记录成为聊天沉淀 | Obsidian 风格知识库不再是孤立工具，而是聊天内容、想法和 Markdown 记录的沉淀处。 | 用户能从聊天语境进入记录/知识库；现有 `/note` 能力保留。 |
| 游戏成为聊天房间小应用 | 游戏继续复用 Channel 系统，作为聊天房间里的实时互动能力。 | 用户能从聊天或房间进入游戏，游戏事件仍走 `game.*` channel。 |
...
| 知识库 | `/note/` | `src/features/note/NotePage.tsx`、`src/components/MilkdownEditor.tsx`、`src/features/note/noteVaultApi.ts`、`server/src/http/routes/noteVaultRoutes.js` | 定位为“记录”；后续接入聊天收藏/保存入口。 |
| 游戏房间 | `/game/gandengyan/`、`/game/zhajinhua/` | `src/hooks/useGameRoom.ts`、`server/src/core/gameRoom.js`、`src/features/game/**` | 作为聊天房间小应用；继续复用 `game.<gameId>.<roomCode>` channel。 |
...
| 聊天内能力 | 记录 | 保存消息、写 Markdown、知识库 | `/note/` + IndexedDB / 本地 Markdown vault |
| 聊天内能力 | 游戏 | 在房间里玩 | `game.*` channel + 游戏事件 JSON |
...
| P4：记录接入聊天 | 把知识库解释为聊天里的“记录”。 | 新增从消息保存到记录的入口；记录页继续支持 Obsidian 风格 Markdown vault；不改变现有 vault API。 | `src/features/chat/ChatPage.tsx`、`src/features/note/NotePage.tsx`、`src/features/note/noteVaultApi.ts`、`src/lib/i18n/messages/note.ts` | `node --test server/tests/unit/noteVault.test.js server/tests/unit/accountBackup.test.js`；手动验证保存消息到记录。 |
| P5：游戏作为聊天房间小应用 | 让游戏从聊天房间进入，而不是孤立页面。 | 在聊天详情或房间操作里提供“开始游戏”；游戏仍使用 `game.<gameId>.<roomCode>` channel，不新增独立后端协议。 | `src/hooks/useGameRoom.ts`、`src/features/game/**`、`src/components/GameSidebar.tsx`、`server/src/core/gameRoom.js` | `node --test server/tests/unit/gameRoom.test.js server/tests/unit/gandengyan.test.js server/tests/unit/zhajinhua.test.js`；两端加入房间并同步事件。 |
| P6：验收口径切换 | 新 MVP 从“文件闭环”升级为“聊天闭环 + 文件附件闭环”。 | 更新 `docs/acceptance.md`，保留原文件协议回归；新增 P2P 聊天、附件、记录、游戏验收路径。 | `docs/acceptance.md`、必要测试文档 | `npm run test:protocol`、`npm run test:frontend`、相关后端单测；人工跑完整聊天场景。 |
...
7. 用户能把重要消息或想法保存到记录/知识库。
8. 用户能从聊天或房间进入游戏，游戏事件通过 Channel 同步。
...
| 副标题 | 从聊天开始，直接传文件、做记录、开游戏房间 | P2P 微信、微信替代品、匿名聊天平台 |
| 文件 | 在聊天里直接传文件，下载后还能继续帮对方传播 | 免费网盘、永久保存、不限条件下载 |
| 记录 | 把聊天里的重要内容沉淀成 Markdown 记录 | 云端知识库、永久同步 |
| 游戏 | 在聊天房间里开一局 | 独立游戏平台 |
