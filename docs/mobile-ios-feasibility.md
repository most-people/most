...
本文用于验证 MostBox 的完整前台 P2P 节点能否在真实 iPhone 上成立。移动端导航采用“消息 / 文件 / 笔记 / 我”四入口；消息页采用会话列表→聊天详情的统一交互，搜索、加号、长按菜单和返回行为与 Android 保持一致。该导航只改变展示层，频道 ID、文件 CID、P2P 下载校验和前台做种约束保持不变。iOS 验证通过前，不开始大规模移动端跨平台重构；技术验证通过也不等于已经满足正式商店合规要求。
