...
安装后确认默认进入“文件”页，底部显示“消息 / 文件 / 笔记 / 我”四个入口：消息进入频道会话，文件包含文件库与传输，笔记进入知识库，“我”集中节点状态、账号、主题、隐私与 Web3。节点状态和本机/远程连接从“我”页进入，并最终显示当前连接状态。

消息入口的真机验收顺序为：会话列表（搜索、置顶、未读、免打扰、长按菜单）→ 点击会话进入详情 → Android 返回回到列表并保留位置；右上角 `+` 只负责创建或加入频道，聊天输入栏不出现在列表页。切换到其他 Tab 或重启应用时，传输、做种和频道订阅继续运行。
