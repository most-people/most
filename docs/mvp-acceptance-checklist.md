...
| 云盘误解清理 | 主应用不出现云端订单、赔付、付费保种市场叙事                            | `rg -n -e '云端订单                  | 赔付 | 付费保种 | 保种市场' app components` |
...
| 检查项       | 通过标准                                                               | 入口                                    |
| ------------ | ---------------------------------------------------------------------- | --------------------------------------- |
| 笔记定位文案 | 计划文档明确笔记云备份只覆盖笔记数据，不是 MostBox 文件云盘            | `docs/plan/独立工具箱与笔记.md`         |
...
| CID 边界     | `calculateNoteCid()` 只用于笔记 raw CID，不进入 `most://` 文件分享协议 | `docs/plan/协议基线实现入口清单.md`     |
