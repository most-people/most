...
| 旧加密笔记迁移       | 临时页面在手动备份和二次确认后，将可解密旧文章覆盖保存为普通 Markdown，失败项保持原样       | `/note/decrypt/`                              |
...
1. 使用旧版本准备一个普通加密笔记、一个空白加密笔记和一个损坏的 `mp://1` 内容；浏览器 IndexedDB 与桌面 Markdown 目录各准备一组。
2. 登录创建这些笔记时的原账号，从知识库进入 `/note/decrypt/`；页面应分别列出浏览器和桌面目录候选项，损坏内容标为无法解密。
3. 未勾选“已完成手动备份”时操作按钮必须禁用；勾选后仍需二次确认才能覆盖保存。
4. 执行后，普通和空白加密笔记应成为可直接读取的 Markdown，并更新浏览器笔记 CID；损坏或密钥不匹配的密文保持原样。
5. 扫描后用外部编辑器修改桌面文件，再执行迁移；该文件不得被迁移页面覆盖。账号本地导出和云备份仍保持整体加密。
