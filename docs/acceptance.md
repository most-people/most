...
6. 进入 `/chat/` 打开频道，点击随机按钮生成 22 位 base64url ID；复制 `/chat/#<channelId>` 链接并在另一节点直接打开，确认文本、附件和语音仍通过同一 P2P Channel 同步。
...
`/app/` 是文件库和传输管理入口，也是本机节点首页的一等入口；底层文件协议仍是 MVP 的硬约束，不能因为首页改造而弱化。
...
1. 点击「打开频道」，确认只出现一个频道输入框；它接受裸频道 ID 和完整 `/chat/#` 分享链接。
2. 点击输入框旁的随机按钮，确认生成 22 位、字符集为 `[A-Za-z0-9_-]` 的随机 ID；也允许输入 3-30 位自定义 ID，短 ID 显示可猜测风险提示。
3. 打开随机 ID 会创建频道，打开已有 ID 会加入对应频道；界面不再区分“创建”和“加入”。
4. 在聊天设置复制分享链接，确认格式为当前 origin 加 `/chat/#<channelId>`，不出现 `?channel=`。
5. 刷新该链接或在新窗口直接打开，登录和节点就绪后应自动加入。
6. 两个节点打开同一 ID 后验证文本、附件和语音；打开不同 ID 的节点不应收到消息或进入同一 topic。
7. 确认旧普通频道仍能打开，首页不再显示好友入口，`/friend/` 不再是应用路由。
8. 明确安全边界：消息记录和语音信令不做应用层加密；知道频道 ID 的人、已加入 peer、daemon 和数据目录访问者均可读取明文，ID 泄露后需新建频道。
...
| 能力频道     | 单一“打开频道”入口；可输入 ID/链接或生成 128 位随机 ID；自定义短 ID 提示风险；不宣称端到端加密            | `src/features/chat/ChatPage.tsx`、`src/lib/chatRoom.js` |
