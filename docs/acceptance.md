...
本地源码验收建议使用 Node.js >= 24。当前 TanStack Start static prerender 前端和 Electron 43 开发/打包都建议 Node.js >= 24。
...
桌面端首页和工作区使用左侧入口轨道、中间列表和右侧内容区；移动端使用“消息 / 文件 / 笔记 / 我”四个入口。首次启动仍进入节点定位门户，后续启动可恢复上次工作区；`most://` 与 `/chat/#<channelId>` 深链优先。工作区的会话未读、置顶、免打扰和草稿只保存在本机，不引入好友或公开内容目录。

聊天能力频道回归：

1. 进入“消息”后先看到纵向会话列表，搜索框和右上角 `+`；输入框只在进入聊天详情或选择创建/加入频道后出现。
...
7. 在会话行长按验证置顶、免打扰和标为已读；切换节点或重启后验证每频道草稿、未读和列表位置按节点/身份隔离恢复。
8. 明确安全边界：仅声明派生 topic 或发送错误、旧连接的证明时，不返回频道 metadata、writer core key 或 presence；完成绑定当前连接的原始频道 ID 持有证明后才同步。
9. 消息记录和语音信令不做应用层加密；知道频道 ID 的人、已加入 peer、daemon 和数据目录访问者均可读取明文，ID 泄露后需新建频道。
...
| 能力频道     | 会话列表→详情；支持搜索、加号创建/加入、长按置顶/免打扰/已读；可输入 ID/链接；ID 不区分大小写；不宣称端到端加密 | `src/features/chat/ChatPage.tsx`、`src/lib/chatRoom.js` |
...
| 检查项          | 通过标准                                                                                                              | 入口                           |
| --------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| 节点入口        | 默认进入文件，底部显示“消息 / 文件 / 笔记 / 我”；文件入口包含传输，节点状态从“我”页进入                               | Android App                    |
