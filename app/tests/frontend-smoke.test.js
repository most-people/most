...
describe('frontend smoke checks', () => {
  ...
  it('keeps Gan Deng Yan long games, bot turns, and voice clips wired', () => {
    const source = readSource('app/gandengyan/page.tsx')

    assert.match(source, /const gameMessageLimit = 2000/)
    assert.match(source, /limit: gameMessageLimit/)
    assert.match(source, /refreshOnMessage: true/)
    assert.match(source, /getChannelMessages\(nextChannelName, gameMessageLimit\)/)
    assert.match(source, /afterActionId/)
    assert.match(source, /currentPlayer\.seat/)
    assert.match(source, /当前账号不在本局/)
    assert.match(source, /新开一局/)
    assert.match(source, /离开游戏/)
    assert.match(source, /resetScores/)
    assert.match(source, /重置分数/)
    assert.doesNotMatch(source, /牌局记录/)
    assert.doesNotMatch(source, /room\.log\.map/)
    assert.match(source, /`single-\$\{localValue\}`/)
    assert.match(source, /`pair-\$\{localValue\}`/)
    assert.match(source, /voiceAssetVersion = '20260602-face-rank-remap'/)
    assert.match(source, /localRankVoiceKey/)
    assert.match(source, /j: 'q'/)
    assert.match(source, /q: 'k'/)
    assert.match(source, /k: 'a'/)
    assert.match(source, /a: 'j'/)
  })
})
