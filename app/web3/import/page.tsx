...
export default function Web3ImportPage() {
  ...
  const runImport = async () => {
    ...
    try {
      const data = await api
        ...
        .json()
      setResult(data)
      addToast(data.success ? '导入完成' : '导入完成，部分文件失败', data.success ? 'success' : 'error')
  ...
  return (
    <AppShell
      sidebar={() => (
        <>
          <Link href="/web3" className="sidebar-header sidebar-header-link">
            <ArrowLeft size={18} />
            <h1>MOST PEOPLE</h1>
          </Link>
      ...
      headerTitle={<h2 className="header-title">用户数据导入</h2>}
      headerRight={
        <button
    ...
    >
      <div className="web3-page">
        <div className="web3-container">
          <section className="web3-key-card">
            <div className="web3-key-card-header">
              <span className="web3-key-card-icon">
                <FileJson size={18} />
              </span>
              <span className="web3-key-card-title">导入 MostBox 元数据</span>
            </div>
                ...
                <>
                        ...
                        <div className="web3-import-failures">
                          ...
                          {check.failures.slice(0, 20).map((failure, index) => (
                            <p key={`${failure.cid || failure.fileName || index}`}>
                              {failure.fileName || failure.cid || '导入包'}: {failure.error}
                            </p>
                          ))}
                        </div>
                      )}
                  ...
                  {result && (
                    <div className="web3-import-result">
                      <strong>
                        已导入 {result.importedFiles} 个文件，{result.importedTrashFiles} 个回收站项
                      </strong>
